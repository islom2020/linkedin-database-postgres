create function get_skill_by_user_id(ii_user_id character varying)
    returns TABLE
            (
                skill_id            uuid,
                skill_name          character varying,
                skill_category_id   uuid,
                skill_category_name character varying
            )
    language plpgsql
as
$$
...
alter function get_skill_by_user_id(varchar) owner to postgres;
